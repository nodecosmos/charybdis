...
pub(crate) fn update_model_rule(
    struct_name: &Ident,
    args: &CharybdisMacroArgs,
    fields: &CharybdisFields,
) -> TokenStream {
    let struct_name_str = struct_name.to_string().to_snake_case();
    let macro_name_str = format!("update_{}", struct_name_str);
    let macro_name = parse_str::<TokenStream>(&macro_name_str).unwrap();
    ...
    let expanded = quote! {
        ...
        macro_rules! #macro_name {
            ($query: literal, $values: expr) => {
                ...
                <#struct_name as charybdis::operations::Update>::update_with_value(concat!(#update, $query, #query_str), $values);
            }
        }
